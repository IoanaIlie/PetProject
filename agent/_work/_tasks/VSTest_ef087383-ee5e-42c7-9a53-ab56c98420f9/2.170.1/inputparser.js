...
function parseInputsForDistributedTestRun() {
...
exports.parseInputsForDistributedTestRun = parseInputsForDistributedTestRun;
function parseInputsForNonDistributedTestRun() {
    let inputDataContract = {};
...
exports.parseInputsForNonDistributedTestRun = parseInputsForNonDistributedTestRun;
function getTestSelectionInputs(inputDataContract) {
    ...
    switch (inputDataContract.TestSelectionSettings.TestSelectionType) {
        case 'testplan':
            ...
            inputDataContract.TestSelectionSettings.TestPlanTestSuiteSettings.TestSuites = new Array();
...
function getTfsSpecificSettings(inputDataContract) {
...
function getTargetBinariesSettings(inputDataContract) {
...
function getTestReportingSettings(inputDataContract) {
...
function getTestPlatformSettings(inputDataContract) {
    ...
    if (vsTestLocationMethod === utils.Constants.vsTestVersionString) {
        ...
        if (utils.Helper.isNullEmptyOrUndefined(vsTestVersion)) {
            ...
            if (matches && matches.length !== 0) {
                ...
                utils.Helper.publishEventToCi(constants_1.AreaCodes.TOOLSINSTALLERCACHENOTFOUND, tl.loc('toolsInstallerPathNotSet'), 1041, false);
            ...
            if (utils.Helper.isNullOrWhitespace(inputDataContract.VsTestConsolePath)) {
                ...
                utils.Helper.publishEventToCi(constants_1.AreaCodes.SPECIFIEDVSVERSIONNOTFOUND, 'Tools installer task did not complete successfully.', 1040, true);
...
function getLoggingSettings(inputDataContract) {
...
function getProxySettings(inputDataContract) {
    // Get proxy details
...
function getDistributionSettings(inputDataContract) {
    ...
    switch (distributionType) {
        case 'basedOnTestCases':
            inputDataContract.DistributionSettings.DistributeTestsBasedOn = constants_1.DistributionTypes.NUMBEROFTESTMETHODSBASED;
        ...
        case 'basedOnExecutionTime':
            inputDataContract.DistributionSettings.DistributeTestsBasedOn = constants_1.DistributionTypes.EXECUTIONTIMEBASED;
        ...
        case 'basedOnAssembly':
            inputDataContract.DistributionSettings.DistributeTestsBasedOn = constants_1.DistributionTypes.ASSEMBLYBASED;
            break;
    }
    return inputDataContract;
}
function getExecutionSettings(inputDataContract) {
...
function getDiagnosticsSettings(inputDataContract) {
...
function getTiaSettings(inputDataContract) {
    // TIA stuff
...
function getRerunSettings(inputDataContract) {
...
function getTIALevel(fileLevel) {
    if (fileLevel && fileLevel.toUpperCase() === 'FALSE') {
        return false;
    }
    return true;
}
function getTestPlatformPath(inputDataContract) {
    const vsTestVersion = tl.getInput('vsTestVersion');
    ...
    const vsVersion = parseFloat(vsTestVersion);
    if (vsVersion === 16.0) {
...
function logWarningForWER(runUITests) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const isEnabledInHKCU = yield isDontShowUIRegKeySet(regPathHKCU);
        const isEnabledInHKLM = yield isDontShowUIRegKeySet(regPathHKLM);
        if (!isEnabledInHKCU && !isEnabledInHKLM) {
            tl.warning(tl.loc('DontShowWERUIDisabledWarning'));
        }
    });
}
function isDontShowUIRegKeySet(regPath) {
    const defer = Q.defer();
...
function addResultsDirectoryToTelemetry(resultsDir) {
...
function setIsServerBasedRun(isServerBasedRun) {
    serverBasedRun = isServerBasedRun;
}
exports.setIsServerBasedRun = setIsServerBasedRun;
function setEnableDiagnosticsSettings(enableDiagnosticsSettingsFF) {
...
exports.setEnableDiagnosticsSettings = setEnableDiagnosticsSettings;
...
exports.getDtaInstanceId = getDtaInstanceId;
